...
function ShowProf (props) {
  ...
  const handleDelete = async () => {
    ...
    const response = await fetch('/obeapi/delProf', {
      method: 'DELETE',
      headers,
  ...
  useEffect(() => {
    const showProf = async() => {
      const response = await fetch('/obeapi/listaProf/'+nameSearch, {method: 'GET'});
      ...
      <Conteudo>
        {modal && 
          <ModalDiv>
            ...
            <h1>Deseja realmente excluir o professor {nomeDel}?</h1>
            <span>
        ...
        <hr></hr>
        <Span><button onClick={() => window.location.replace('/newProf')}><AiOutlineUserAdd/> Novo professor</button></Span>
        <Table>
            <tr>
              <th type="id">Id</th>
            ...
            <tbody>
            {
              data.map(function(data){
                return <tr><td>{data.idProfessor}</td> <td>{data.nome}</td> <td>{data.cpf}</td> <td>{data.email}</td> <td>{data.telefone}</td><td><button id={data.idProfessor} onClick={() => handleEdit(data.idProfessor)}><FaPencilAlt/></button></td><td><button onClick={() => toggle(data.idProfessor,data.nome)}><FaTrashAlt/></button></td></tr>;
              }.bind(this))
            }
          </tbody>
        </Table>
...
export default withRouter(ShowProf);